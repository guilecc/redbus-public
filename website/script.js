const translations = {
    en: {
        ...
        feat_orch_desc: "Dynamic routing. Use fast models for planning (Maestro) and powerful models like Claude or Gemma 4 for execution (Worker).",
        ...
        feat_llm_desc: "Run Gemma 4 locally with a zero-friction setup. Seamlessly switch to OpenAI, Gemini, or Anthropic for massive cloud intelligence.",
    ...
    pt: {
        ...
        feat_orch_desc: "Roteamento dinâmico. Use modelos rápidos para planejar (Maestro) e potentes como Claude ou Gemma 4 para executar (Worker).",
        ...
        feat_llm_desc: "Rode Gemma 4 localmente com instalação simplificada. Conecte-se também à OpenAI, Gemini e Anthropic para inteligência em nuvem.",
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Fetch environment variables for Github URL
    fetch('.env')
        .then(res => {
        ...
        .then(text => {
            ...
            const githubLine = lines.find(line => line.startsWith('GITHUB_URL='));
            if (githubLine) {
                const url = githubLine.split('=')[1].trim();
                document.getElementById('github-link').href = url;
            }
        })
        .catch(err => {
            console.warn("Using fallback github url due to missing .env. Note: you might need to host the website for fetch to work locally.", err);
            document.getElementById('github-link').href = 'https://github.com/guilecc/redbus';
        });
});
