...
async function processWithWhisper(audioBuffer: Buffer, mimeType: string, openAiKey: string, configs: any): Promise<MeetingAnalysis> {
  ...
  // Step 2: Analyze transcript with configured worker LLM
  const summary_json = await analyzeTranscript(transcript, configs);
...
async function analyzeTranscript(transcript: string, configs: any, isLocalTranscript = false): Promise<MeetingAnalysis['summary_json']> {
...
Return ONLY valid JSON with this exact schema:
{
  ...
  let raw = '';
  if (workerModel.includes('gemini') && configs.googleKey) {
    ...
    const res = await fetchWithTimeout(url, {
      ...
      body: JSON.stringify({ systemInstruction: { parts: [{ text: systemPrompt }] }, contents: [{ parts: [{ text: userPrompt }] }], generationConfig: { responseMimeType: 'application/json' } })
    }, 120_000);
    if (!res.ok) throw new Error(`Gemini error: ${res.status}`);
    const d = await res.json(); raw = d.candidates?.[0]?.content?.parts?.[0]?.text || '';
  } else if (workerModel.includes('claude') && configs.anthropicKey) {
    ...
    const res = await fetchWithTimeout('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({ model: workerModel, response_format: { type: 'json_object' }, messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: userPrompt }] })
    }, 120_000);
    if (!res.ok) throw new Error(`OpenAI error: ${res.status}`);
    const d = await res.json(); raw = d.choices?.[0]?.message?.content || '';
  } else {
    throw new Error(`No API key available for model: ${workerModel}`);
  }
 ...
 */
export async function analyzeTranscriptFromText(
  transcript: string,
  configs: { googleKey?: string; openAiKey?: string; anthropicKey?: string; workerModel?: string;[k: string]: any }
): Promise<MeetingAnalysis['summary_json']> {
  return analyzeTranscript(transcript, configs, true);
}
...
export async function processAudio(
  ...
  configs: { googleKey?: string; openAiKey?: string; workerModel?: string;[k: string]: any }
  ...
  switch (engine) {
    case 'gemini': {
      ...
      const model = configs.workerModel?.includes('gemini') ? configs.workerModel : 'gemini-2.0-flash';
    ...
    case 'whisper': {
      ...
      return processWithWhisper(audioBuffer, mimeType, configs.openAiKey, configs);
    }
    case 'local': {
